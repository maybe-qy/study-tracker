...
## 2026-08-01 — v5.1.1 平台名称修正 + 推广文案重写

### 平台名称标准化
- **统一命名**：全平台统一为"豆包工作任务模式"和"超级元宝"，替换所有"豆包 Office Task""元宝超级智能体"等旧称
- **README 重写**：从功能列表改为痛点驱动风格，增加故事化开场和超级元宝专属快速开始入口
- **推广文案拆分**：分为版本 A（痛点驱动/朋友圈/家长群）和版本 B（简洁版/小红书/微博）
- **GitHub 模板更新**：Issue/PR 模板重写为更简洁自然的格式
- **一致性修复**：遗留的"8 份报告"过时引用全部改为 2 报告体系（个人总览 + 单科追踪）

## 2026-08-01 — v5.1.0 置信度系统优化 + 方法族并行化

### 置信度体系优化
- **D→C 降级修复**：无参照数据的粗略估算从 D 级改为 C 级，D 级仅保留在完全无数据场景
- **动态置信度升级**：融合阶段 3+ 方法收敛（分歧 ≤ 3分）时自动升级置信度一级（C→B, B→A）
- **误差区间对齐**：融合后误差区间基于融合后置信度而非主方法原始置信度
- **木桶原理置信度**：族①双模块换算法采用"取最低置信度"逻辑，修复虚假 A 级问题

### 方法族并行化
- **族②改为并行**：分数线对照法/排名锚定法/校内排名对照法全并行，都参与融合
- **族③改为并行**：人数校准法/校排阈值估算法全并行，都参与融合
- **跨族同源去重**：新增 DATA_SOURCES/FAMILY_MAP/METHOD_PRIORITY，同族同源只保留最高置信度方法
- **方法④按科目数衰减**：单科排名对照法在融合中权重随科目数衰减（1科→0.33, 2科→0.67）

### 报告增强
- **分歧指示灯**：个人档案页新增 ● 指示灯（绿/黄/红），直观显示方法一致性
- **加权趋势分析**：新增 `compute_trend_weighted()`，趋势方向按置信度加权计算
...
- 模块一：语数英用校内特控线/浙大线 → 高考目标 341/382 比例换算
- 模块二：选科三级优先级（双线换算A→单线换算B→跨次回退/赋分C）
- 高考参考目标基于 2026 浙江高考各科均衡目标的中点
