...
报告文件生成在 `output/` 目录后，**用户无法直接访问你的云端文件系统**。你必须用以下方式之一让用户拿到报告：

**方式一（推荐）：直接输出HTML内容**
逐个读取 `output/` 目录下的 `.html` 文件，把文件内容用代码块输出给用户：
```
这是你的【个人档案】报告，复制以下内容保存为 .html 文件用浏览器打开：
\`\`\`html
（文件完整内容）
\`\`\`
```
...
**方式二：如果支持文件下载**
如果你的平台支持文件下载/附件功能，把 `output/` 下的所有 .html 文件作为附件发送给用户。

**绝对不要**只说"文件在 output/ 目录下，用浏览器打开"——用户无法访问你的文件系统，这样说等于没有输出。
