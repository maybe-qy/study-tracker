...
| 数据 | 怎么拿 | 有什么用 |
|------|--------|---------|
| **本次考试特控线** | 问老师/年级组 | 有它 → A级等效分（±5分），可直接对标浙大665 |
| **全市/联盟排名+总人数** | 联考统考成绩单 | 有它 → A级等效分（±5分） |
| 只有校排名 | 学校成绩单 | 只能C级（±15分），**不算了，告诉用户去拿特控线** |

**关键原则**：如果用户没提供特控线也没提供市排名，**不要算C级等效分**。明确告诉用户：
"这次考试的等效分暂时算不了——需要你问老师要一个数据：**本次考试的特控线**（就是学校划的特殊类型招生控制线分数）。有了这个分数，我就能帮你换算到高考尺度，误差只有±5分。"
...
**方式一：直接说分数**
```
语文102 数学128 英语110 物理原始70赋82 化学68赋79 技术88赋84 总分576 校排名150/600 特控线542.5
```

**方式二：分科说**
```
语文102，数学128，英语110，物理赋了82，化学赋了79，技术赋了84，特控线542.5
```
...
**特控线提醒**：如果用户没说特控线也没说市排名，录入完成后提醒一次："这次有特控线或市排名吗？有的话等效分能算到±5分精度。"
...
- **B级（±10分）**：有校内门槛人数数据，精度中
- **不算**：只有校排名时不算等效分，因为±15分跨度过大，没有决策价值

计算需要的关键数据（按获取难度排序）：
1. **本次考试特控线**（问老师要）→ A级，最推荐
2. **全市/联盟排名+总人数**（联考成绩单）→ A级
3. 校内特控线上线人数（学校内部数据）→ B级
