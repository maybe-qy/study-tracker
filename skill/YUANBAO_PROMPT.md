...
> 本文件说明 study-tracker 在**元宝超级智能体**（区别于豆包工作任务模式）下的适配要点。
> **核心差异**：豆包任务模式是自动化沙箱（后台静默执行命令），元宝是对话式智能体带内置工具集（每条操作用户可见，工具能力不同）。
...
### 1.2 元宝内置的常用工具（需要利用，不要绕开）

元宝通常有以下内置工具，**不要自己另外实现**：
...
```
豆包模式：用户说"帮我录入" → AI 自动执行全部命令 → 输出结果
元宝模式：用户说"帮我录入" → AI 引导获取信息 → 用代码执行工具运行脚本 → 输出结果
```

**关键原则**：元宝模式下，不要试图让用户手动操作（如"请打开终端运行以下命令"），而是**善用元宝自带的代码执行工具**在后台完成。
...
豆包任务模式会自动：`git clone → pip install → python setup_workspace.py`

### 2.2 元宝该怎么做

**方式一：代码执行工具可用（推荐）**

元宝有代码执行工具时，用该工具按顺序执行：
...
如果元宝没有代码执行能力，**不要尝试运行 Python 脚本**。改为：
- 引导用户在本地运行项目
- 提供明确的本地运行指令
- 用户录入成绩后，手动计算等效分
...
如果元宝有文件下载功能，直接提供报告文件链接。如果没有，用代码块输出 HTML 内容：
...
- 元宝模式下，用户的数据文件（Excel）需要通过上传工具获取
- 如果用户没有上传数据文件，引导用户直接说分数（元宝记录后写 JSON 文件）
- 不要假设文件系统上有历史数据

### 4.3 进度展示

因为元宝的每条工具调用用户可见，要在工具调用前先说明要做什么：
