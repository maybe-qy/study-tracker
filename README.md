...
| **等效分计算** | 7 种方法按优先级自动选择，加权融合，标注置信度和误差区间 |
...
系统会自动降级到人数校准法（B 级）。推荐补充门槛数据（特控线上线人数）以启用 B 级方法。若无任何 A/B 级方法可用，返回 insufficient_data。
...
| [计算方法详解](skill/references/calculation_methods.md) | 7 种方法公式与边界条件 |
